...
/**
 * Custom transients' implementation by WPForms.
 *
 * @since 1.0.0
 */
class Transient {
	 ...
	 * @var string
	 */
	const OPTION_PREFIX = '_commentswp_transient_';
	 ...
	 * @var string
	 */
	const TIMEOUT_PREFIX = '_commentswp_transient_timeout_';
	 ...
	 * @since 1.0.0
	 *
	 * @param string $transient  Transient name. Must be 164 characters or fewer.
	...
	public static function set( $transient, $value, $expiration = 0 ) {
		...
		if ( self::get_option( $transient ) === false ) {
			return self::add( $transient, $value, (int) $expiration );
		}

		return self::update( $transient, $value, (int) $expiration );
	}
	 ...
	 *
	 * @param string $transient Transient name. Expected to not be SQL-escaped.
	 *                          Must be 164 characters or fewer.
	 * @param mixed  $value     Transient value. Must be serializable if non-scalar.
	 *                          Expected to not be SQL-escaped.
	 * @param int $expiration   Optional. Time for which not to expire, in seconds.
	...
	public static function delete_all() {
		...
		return (int) $wpdb->query( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching
			$wpdb->prepare(
